...
void GOL_B_Enc::scramble(Cryptographic_GOL_Board* gol_board)
{
    ...
    for (int i = 0; i < 64; i+=2) { 
            (*gol_board).board[i][0] = (*gol_board).board[i][0]<<16;
            (*gol_board).board[i+1][0] = (*gol_board).board[i][0]>>16;
    }
    (*gol_board).steps(16);
}
...
void GOL_B_Enc::encrypt(std::string path) {
    ...
    for (int i = 0; i<64; i++) {key_map[i] = gol_board.board[i][0];}
    u_int64_t hash_map[64];
    for (int i = 0; i<64; i++) {hash_map[i] = hash.gol_board.board[i][0];}
    ...
    while (input_size >= BLOCK_SIZE) {
        ...
        for (int i = 0; i < 64; i++) { 
            buffer[i] ^= key_map[i] ^ gol_board.board[i][0];
        }
        output_stream.write(reinterpret_cast<char*>(&buffer[0]), 64 * sizeof(u_int64_t));
        gol_board.set_board(buffer);
        scramble(&gol_board);
...
void GOL_B_Enc::decrypt(std::string path) {
    ...
    input_stream.read(reinterpret_cast<char*>(&buffer[0]), BLOCK_SIZE);
    for (int i = 0; i < 64; i++) { 
        hash_map[i] = key_map[i] ^ buffer[i];
    }
    hash.gol_board.set_board(hash_map);
    ...
    while (input_size >= BLOCK_SIZE) {
        ...
        for (int i = 0; i < 64; i++) { 
            buffer[i] = in_buffer[i] ^ key_map[i] ^ gol_board.board[i][0];
        }
